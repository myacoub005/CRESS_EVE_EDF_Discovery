...
IFS=,
tail -n +2 metadata/Ascomycota_1_NCBI_ACC.csv | sed -n ${N}p | while read GENUS PHY SP STR ACC NA
do
...
cat Results/Basidiomycota.TRUE_REP.tsv | cut -f1 >> $OUTDIR/${OUTNAME}.REP.hits.txt

seqtk subseq $INDIR/$ACC/${OUTNAME}_CRESS.proteins.fasta $OUTDIR/${OUTNAME}.REP.hits.txt >> $OUTDIR/${PHY}_REP.aa.fasta
seqtk subseq $INDIR/$ACC/${OUTNAME}.protein.faa $OUTDIR/${OUTNAME}.REP.hits.txt >> $OUTDIR/${PHY}_REP.aa.fasta
